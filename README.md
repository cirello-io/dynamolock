# DynamoDB Lock Client for Go v5

[![Build status](https://github.com/cirello-io/dynamolock/actions/workflows/v5.yml/badge.svg)](https://github.com/cirello-io/dynamolock/actions/workflows/v5.yml)
[![GoDoc](https://pkg.go.dev/badge/cirello.io/dynamolock/v5)](https://pkg.go.dev/cirello.io/dynamolock/v5)
...
```sh
$ go get -u cirello.io/dynamolock/v5
```
...
import (
	...
	"github.com/aws/aws-sdk-go-v5/aws"
	"github.com/aws/aws-sdk-go-v5/config"
	"github.com/aws/aws-sdk-go-v5/service/dynamodb"
	"github.com/aws/aws-sdk-go-v5/service/dynamodb/types"
)
...
expiring (it does this by calling the `SendHeartbeat(ctx, ...)` method in the lock
client.) This will ensure that as long as your application is running, your
locks will not expire until you call `ReleaseLock(ctx, ...)` or `lockItem.Close(ctx)`

### Read the data in a lock without acquiring it
You can read the data in the lock without acquiring it, and find out who owns
the lock. Here's how:
```Go
lock, err := lockClient.Get(context.Background(), "kirk");
```
...
For an IAM role to take full advantage of `dynamolock/v5`, it must be allowed to
